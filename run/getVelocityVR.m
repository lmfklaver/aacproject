...
addParameter(p,'VR',1.86,@isnumeric);
...
if max(diff(unique(pos_scaled)))>std(unique(pos_scaled));
    warning('There is a likely a jump in VR conversion to voltage, need to remove jump - there is a dirty fix')
    for i=1:5
    thr=max(diff(unique(pos_scaled)));
    thri=find(diff(unique(pos_scaled))==thr)+1;
    vals=unique(pos_scaled);
    difvolt=vals(thri);
    toohigh=find(pos_scaled>=difvolt);
    pos_scaled(toohigh)=pos_scaled(toohigh)-thr;
    end
end

convfactor=(VR*100)/max(pos_scaled)
pos_in_cm   = pos_scaled*convfactor; % normalize it to size of wheel

% additive positions (roll-out-the-wheel)
thr_diff    = 10*std(pos); % hardcoded
stpidx      = find(abs(diff(pos_in_cm))> thr_diff); % is taking the wheel resets

%% Lap times workflow
thr_diff_cm = 10*std(diff(pos_in_cm));
...
stpidx=peakneg(1:end)';
stridx=(peakneg(1:end)+1)';
...
posToNeg = 0%(trN_idx(1)-trP_idx(1))> 1;%pos(trP_idx(1))> pos(trP_idx(1)+1); % this means that the position value decreases
negToPos = 1%(trP_idx(2)-trN_idx(1))> 1;%pos(trP_idx(1))< pos(trP_idx(1)+1);


pos_in_cm=pos_in_cm_fix
if  posToNeg
    for idx = 1:length(stridx)

        if stridx(idx) < stridx(end)
            pos_in_cm(stridx(idx)+1:stridx(idx+1)) = ...
                pos_in_cm_fix(stridx(idx)+1:stridx(idx+1)) -  ...
                ((pos_in_cm_fix(stridx(idx)+1) - pos_in_cm_fix(stridx(idx))));

        elseif stridx(idx) == stridx(end)
            pos_in_cm(stridx(idx)+1:end) = ...
                pos_in_cm_fix(stridx(idx)+1:end) - ...
                ((pos_in_cm_fix(stridx(idx)+1) - pos_in_cm_fix(stridx(idx))));
        end
...
elseif negToPos
    ...
    for idx = 1:length(stpidx)
        if stpidx(idx) < stpidx(end)
            pos_in_cm(stpidx(idx)+1:stpidx(idx+1)) =...
                pos_in_cm_fix(stpidx(idx)+1:stpidx(idx+1)) + ...
                pos_in_cm(stpidx(idx));
        else
            pos_in_cm(stpidx(idx)+1:end) = ...
                pos_in_cm_fix(stpidx(idx)+1:end)+ ...
                pos_in_cm(stpidx(idx));
        end
        hold on
    end
    vel_cm = abs(diff(movmean(pos_in_cm,20)));
end
...
%     vel.laps.posfilt{i}=sgolayfilt(vel.laps.pos{i},2,3);
% end
% figure, hold on,
% for i=1:length(vel.laps.pos),
%     plot(vel.laps.pos{i});
