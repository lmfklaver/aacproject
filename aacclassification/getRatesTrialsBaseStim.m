...
%realigns spikes per unit per trial to start of pulse at 0s

for iUnit = 1:length(spikes.UID)
    % how many spikes are there before pulse onset? 
    baseInd{iUnit} = cellfun(@(a) a<0, spkTimesPerTrial{iUnit},'UniformOutput', false);
    % how many spikes are there after pulse onset? only taking into account
    % duration specified by pulsew (default == 0.1)
