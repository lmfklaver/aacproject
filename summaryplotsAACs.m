...
sessions = [18]%1,2,3,4,5];%,8,9,16,17];% 17;%[8,9,16,17]; %1,2,3,4,5,
...
%                     'binSize', 0.01, 'saveAs', '.ripplepeth.analysis.mat');
            ...
            [status]=InIntervals(ripples.peaks,gd_eps);
            gd_ripplepeaks=ripples.peaks(status);
            for iRip = 1:length(gd_ripplepeaks)
        ...
        allpreIDX = find(STP.post_idx(:,3) == aacs(iallaacs));
        allaacIDX=[allaacIDX;allpreIDX];
        end
        uniqueallaacIDX=unique(allaacIDX);
        meanpresyn=mean(STP.acg_pre(uniqueallaacIDX,:));
        meannopresyn=mean(STP.not_mono_con_to_aac_acg);
        plot(meanpresyn');
        hold on
        plot(meannopresyn');
        box off
        ...
        for q=1:size(spkInstPhaseStruct.binnedHisto,1)
            hold on
            histogram('BinEdges',spkInstPhaseStruct.ph_bin, 'BinCounts',spkInstPhaseStruct.binnedHisto(q,:),'DisplayStyle','stairs')
%         legendselfreq{q}= num2str(spkInstPhaseStruct.freqband(q,:));
%         legend({legendselfreq});
%         legend('boxoff')
        end
        histogram('BinEdges',spkInstPhaseStruct.ph_bin, 'BinCounts',spkInstPhaseStruct.meanbinnedHisto);
        hold off
        %%
        % % % % % % % % % % % % %
...
%            yline(1,':')
        ...
        if doPhaseMap
            ...
            imagesc(ph_portrait.ph_bin,[],nanconvn((ph_portrait.ph_rate(:,1:end-1,iAAC)),k),[min(linearize(ph_portrait.ph_rate(:,1:end-1,iAAC)))...
                max(linearize(ph_portrait.ph_rate(:,1:end-1,iAAC)))])
            
            hold on
            colormap('jet')
            plot(ph_portrait.ph_bin,10+cos(ph_portrait.ph_bin)*10,'w')
            set(gca,'ytick',0:10:length(ph_portrait.freq),'yticklabel',round(ph_portrait.freq(1:10:end)))
            set(gca,'ydir','normal')
...
% %% Start building the Figure
% 
% for iSess = sessions  %%Earl makes edits here to be able to loop through sessions  - EG
%     
%     cd(dirN{iSess})
%     basepath = cd;
%     basename = bz_BasenameFromBasepath(cd);
...
%     % % PulseEpochs
%     pulseEpochs = optoStim.timestamps;
%     gd_eps=get_gd_eps(basepath); %% add to preprocessing
%     [ccginout] = getCCGinout(basepath,gd_eps,'epochname','gd_eps','saveAs','.ccggoodeps.analysis.mat'); %% add to preprocessing
%     % % % % % % % % % % % % %
%     % % runEpochs
%     
%     minRunLength = 3;
...
%         % % Ripple Modulation
%         % % % % % % % % % % % % %
%         if  doCCGRipMacroMicro
%         [ripple_ccg_mac] = getRipCCG(basepath,spikes,'epochs',gd_eps,'ccgbin', 0.02,'ccgdur', .8 ,'saveMat',false);
%         %PlotCCG(ripple_ccg_mac.t,ripple_ccg_mac.ccg(:,[end, aacs],[end, aacs]))
% 
% 
%         [ripple_ccg_mic] = getRipCCG(basepath,spikes,'epochs',gd_eps,'ccgbin', 0.001,'ccgdur', .1 ,'saveMat',false);
%         %PlotCCG(ripple_ccg_mic.t,ripple_ccg_mic.ccg(:,[end, aacs],[end, aacs]))
%         save([basename '.ripple_ccg_macromicro.mat'],'ripple_ccg_mac','ripple_ccg_mic')
%         end
%         
%         
%         %%
